...
if ( class_exists( 'WP_Importer' ) ) {
	class TM_WP_Importer extends WP_Importer {
		...
		var $imageCount = 0;
		var $totalImages = 0;
		var $generate_thumb = false;

		function WP_Import() { /* nothing */
		}

		function TM_WP_Importer( $generate_thumb ) {
			$this->generate_thumb = $generate_thumb;
		}
		...
		function check_writeable() {
			...
			echo '<br /><form action="" method="post" class="retry-form">' .
			     ...
			     '<input type="hidden" value="' . esc_attr( $_POST['demo'] ) . '" name="demo"/>' .
			     '</form>';
		 ...
		 * @param $_tmppath
		 */
		function download_package( $demo, $_tmppath ) {
			...
			if ( ! is_dir( $_tmppath ) ) {
				...
				$package_name = TM_THEME_SLUG . '-' . $demo . '.zip';

				$url = apply_filters( 'thememove_import_package_url', 'http://' . API_SERVER . '/packages/' . $package_name );

				echo '<script type="text/javascript">text_status(\'' .
				     sprintf(
					     wp_kses(
						     __( 'Downloading package ... You can download the package via <a href="%s">this direct link</a>.' ),
						     array(
							     'strong' => array(),
							     'a'      => array(
								     'href' => array()
					...
					if ( ! is_dir( $_tmppath ) ) {
						...
						if ( @mkdir( $_tmppath, 0755 ) ) {
							...
							<form action="" method="post" id="form-refresh">
								...
								<input type="hidden" value="<?php echo esc_attr( $demo ) ?>" name="demo"/>
							</form>
		...
		function get_data( $demo, $type, $unserialize = true ) {
			...
			$file = get_template_directory() . '/inc/import/files/' . $demo . '/' . $type . '.txt';
		...
		/**
		 * Get avaiable widgets in curent size
		 * @return array
		 */
		 ...
		 */
		function read_widget_logic_file( $demo ) {
			// Widget Logic
			$widget_logic_file = get_template_directory() . '/inc/import/files/' . $demo . '/widget_logic_options.txt';
		...
		function import_sidebars( $demo ) {
			$data = json_decode( $this->get_data( $demo, 'sidebars', false ) );
			echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Sidebars</b>' ), array( 'b' => array() ) ) . '\' );</script>';
		 ...
		 * @return array
		 */
		function import_widgets( $demo ) {

			global $wp_registered_sidebars;

			$data = json_decode( $this->get_data( $demo, 'widgets', false ) );
			...
			$wl_options     = $this->read_widget_logic_file( $demo );
		 ...
		 * @return array
		 */
		function import_woocommerce( $demo ) {
			...
			$wc = $this->get_data( $demo, 'woocommerce' );
		 ...
		 * @param $demo
		 */
		function import_rev_sliders( $demo ) {
			...
			$templine = '';
			$lines    = $this->get_data( $demo, 'rev_sliders', false );
		 ...
		 * @param $demo
		 */
		function import_essential_grid( $demo ) {
			...
			$file = get_template_directory() . '/inc/import/files/' . $demo . '/essential_grid.txt';
		...
		function dispatch() {
			...
			$demo    = $_POST['demo'];
			$_x_path = get_template_directory() . DS . 'inc' . DS . 'import' . DS . 'files' . DS . $demo . DS;

			$_tmppath = $_cpath . TM_THEME_SLUG . '-' . $demo . '_tmp';
			...
			$this->download_package( $demo, $_tmppath );
			...
			// import page options
			$this->import_page_options( $demo );

			// import sidebars
			$this->import_sidebars( $demo );

			// import widgets
			$this->import_widgets( $demo );

			// import menus
			$this->import_menus( $demo );

			// import woocommerce
			$this->import_woocommerce( $demo );

			// import essential grid
			$this->import_essential_grid( $demo );

			// import rev sliders demo
			$this->import_rev_sliders( $demo );
		...
		function fetch_remote_file( $url, $post ) {
			...
			$headers      = $http_request->request( $url, array( $upload['file'] ) );

			// request failed
			if ( ! $headers ) {
				@unlink( $upload['file'] );

				return new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'tm-core' ) );
			}
			...
			if ( $headers['response'] != '200' ) {
				...
				return new WP_Error( 'import_file_error', sprintf( __( 'Remote server returned error response %1$d %2$s', 'tm-core' ), esc_html( $headers['response'] ), get_status_header_desc( $headers['response'] ) ) );
			}
